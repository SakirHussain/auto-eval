...
def evaluate_answer_by_rubric_items(
    ...
    for idx, rubric_item in enumerate(rubric_items, start=1):
        ...
        feedbacks_by_item.append({
            "rubric_item": rubric_item,
            "clarification_feedbacks": clar_raw["response"] if clar_raw and "response" in clar_raw else [],
            "target_guided_feedbacks": target_raw["response"] if target_raw and "response" in target_raw else []
        })
